...
def show_table(rows, title, sk):
    ...
    for i, f in enumerate(rows, start=1):
        ...
        print(f"[{i:>2}] 分{f[sk]:>4} | {f['type']:<11} | "
              f"{f['action']:<5} | 命中@{loc}{flag}")
...
def main():
    ...
    while True:
        ...
        print("操作：输入数字编号=看该条详情 | "
              "a=性价比表 b=实测表 c=全量表 | q=退出")
        ch = input(">>> ").strip().lower()
        if ch == "q":
            break
        elif ch in table_map:           # 字母=切表
            current = table_map[ch]
        elif ch.isdigit() and 1 <= int(ch) <= len(rows):
            # 数字=看【当前这张表】的第N条（rows已是当前表的）
            show_detail(rows[int(ch) - 1], vidx, all_items)
        else:
            print("无效输入：数字看详情，a/b/c切表，q退出。")
